...
def add_class_schedule(subject: str, days: str, start_time: str, end_time: str, location: str = ""):
    ...
    try:
        ...
        if check_duplicate_schedule(subject, start_time, end_time, day_names):
            return f"⚠️ {subject} class already exists with same time and days. Skipping duplicate."
...
def create_semester_calendar_debug():
    ...
    try:
        ...
        try:
            ...
            debug_info.append(f"⚠️ Could not fetch existing events: {e}")
...
def create_semester_calendar():
    ...
    try:
        ...
        if skipped_events > 0:
            result_message += f"⚠️ Skipped: {skipped_events} duplicate events\n"
...
def calculate_attendance():
    ...
    try:
        ...
        for subject, stats in subject_stats.items():
            percentage = (stats["attended"] / stats["total"]) * 100 if stats["total"] > 0 else 0
            status = "✅ Good" if percentage >= 75 else "⚠️ Low" if percentage >= 65 else "❌ Critical"
